...
export class ProfilePage {
  ...
  updateProfile(f:NgForm) {
    ...
    self.WebService.updateprofile(data)
    .subscribe(
      (result:any) => {
        loading.dismiss()
        if(result.status == 200) {
					let toast = self.toastCtrl.create({ message : 'Profile Info updated Successfully', duration: 2000  })
					toast.present();
				}else {
  ...
  getCompanyList() {
    let self = this;
    self.WebService.getCompanyList()
    .subscribe(
      (result:any) => {
        if(result.status == 200) {
